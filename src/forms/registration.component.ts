...
export class LocksmithRegistrationComponent extends LitElement {
  ...
  /**
   * Nonce generated by your RP/server and bound to the current
   * verification/session attempt.
   */
  @property() evpNonce: string = crypto.randomUUID();
  ...
  async sendRegistrationRequest() {
    const evpToken = this.evpTokenRef.value?.value || undefined;
    ...
    const body = {
      ...
      // Browser populates this hidden field during form submission
      // when EVP is supported/available.
      evp_token: evpToken,
    };
  ...
  async attemptRegistration() {
    ...
    if (!this.canSignIn()) {
      ...
      if ((this.emailRef.value?.value.length ?? 0) === 0) {
        this.emailRef.value?.focus();
  ...
  /**
   * EVP token is populated by the browser immediately before the submit
   * event, so don't call attemptRegistration() directly.
   */
          ...
          ? html`
              ...
              ${this.settings.OauthProviders.map((provider) => {
                ...
                const customUrlParams =
                  window.locksmith?.injectURLParams?.();
